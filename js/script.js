...
var mouseMoved = false; // Bubble config
...
var bubbleTimer; // For timing

var timer = 0;
var then = Date.now();
var now;
var delta; // Internet Explorer foolery

var ie = "ActiveXObject" in window; // For mapbox
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    mousePos.y = Math.round(e.clientY - window.innerHeight / 2); // When the mouse is first moved on the page, account for its offset to prevent any jarring movement to its position
  ...
  document.body.dispatchEvent(event); // Nav drawer click listeners
  ...
  var numBubbles = minBubbles + Math.random() * (maxBubbles - minBubbles); // numBubbles *= mobileMultiplier; // Mobile devices need fewer bubbles to populate the screen

  if (!mobile && !ie) {
    // Start spawning the bubbles
    generateBubbles(container, numBubbles, minDiameter, maxDiameter, minDepth, maxDepth, minAlpha, maxAlpha); // Start animating them
...
function generateBubbles(element, numBubbles, minDiameter, maxDiameter, minDepth, maxDepth, minAlpha, maxAlpha) {
  ...
  var alpha = (minAlpha + Math.random() * (maxAlpha - minAlpha)).toPrecision(3); // Initialise the bubble

  var bubble = document.createElement("div");
  bubble.classList.add("bubble");
  bubble.style.width = "".concat(diameter, "px");
  bubble.style.height = "".concat(diameter, "px");
  bubble.style.backgroundColor = "rgba(255, 255, 255, ".concat(alpha, ")");
  bubble.style.transform = "translate3d(".concat(posx, "vw, ").concat(posy, "vh, ").concat(depth, "px)");
  bubble.setAttribute("data-posx", posx);
  bubble.setAttribute("data-posy", posy);
  bubble.setAttribute("data-depth", depth); // Add it to the target element
...
function bubbleTick() {
  if (checkVisible(document.querySelector("#parallax"))) {
    // Only animate the bubbles if the container is visible
    // Timing stuff
    now = Date.now();
    delta = now - then; // Breathing / bobbing animation powered by sine curves (thanks GCSE maths)

    if (timer >= Math.PI) {
      timer = -Math.PI;
    }

    var breatheOffset = Math.sin(timer) * breatheIntensity; // More timing stuff
    ...
    for (var i = 0; i < bubbles.length; i++) {
      ...
      bubbles[i].style.transform = "translate3d(".concat(posX, "vw, ").concat(posY, "vh, ").concat(depth, "px)"); // Update each bubble's transform accordingly
    }
...
function serialize(form) {
  ...
  for (var i = 0; i < form.elements.length; i++) {
    var field = form.elements[i]; // Don't serialize unnamed fields, submits, buttons, file and reset inputs, and disabled fields

    if (!field.name || field.disabled || field.type === "file" || field.type === "reset" || field.type === "submit" || field.type === "button") continue; // If a multi-select, get all selected values
